...
cred = credentials.Certificate("/home/pi/Desktop/sortify-sydw2v-firebase-adminsdk-7t5yi-38686b33ef.json")  # Replace with your service account JSON path
    ...
    while True:
        ...
        if key is not None:
            if key.isdigit():
                ...
                # Check phone number against Firestore
                user_ref = db.collection('user').where(field_path='phone_number', op_string=='==', value=phone_number).limit(1)
                user_data = user_ref.get()
                if len(user_data) > 0:
                    lcd.text("Enter Item", 1)
                    print(f"Phone number {phone_number} exists in Firestore.")
                else:
                    lcd.text("Incorrect Number", 1)
                    print(f"Phone number {phone_number} does not exist in Firestore.")
                
                lcd.text("Phone number saved", 2)
                break
            elif key == 'D':
